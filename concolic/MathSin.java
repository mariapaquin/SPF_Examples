...
public class MathSin {
	...
	public double mysin(double x) {
		...
		if (xexp < IEEE_BIAS) {
			...
			/*
			 * Form xm * (pi/2) exactly by doing: (x3,x4) = xm * pi2_hi (x5,x6) = xm *
			 * pi2_lo
			 */
			// >>>>>>>>>>>>>>>>>>>>> split(a1,a2,xm);
		...
		if (x > X_EPS) {
			...
			x2 = x * x;
			x *= ((((((((P)[0] * (x2) + (P)[1]) * (x2) + (P)[2]) * (x2) + (P)[3]) * (x2) + (P)[4]) * (x2) + (P)[5])
					* (x2) + (P)[6]) * (x2) + (P)[7]);
		} else {
			System.out.println("x <= EPS");
			x *= pi2_hi; /* x = x * (pi/2) */
