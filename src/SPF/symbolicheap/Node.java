...
class Node {
	...
	static void printOriginalList(Node head) {
		for (Node t = head; t != null; t = t.original_next) {
			System.out.println("node" + t.hashcode + " elem = " + (Debug.getSymbolicIntegerValue(t.elem)) + " next = "
					+ ((t._symbolic_next) ? "*"
							: ((t.original_next != null) ? "node" + t.original_next.hashcode : "null")));
		}
	}
	...
	static void printOriginalListCycles(Node head) {
		...
		for (Node t = head; t != null; t = t.original_next) {
			if (visited.add(t)) {
				System.out.println("node" + t.hashcode + " elem = " + (Debug.getSymbolicIntegerValue(t.elem))
						+ " next = " + ((t._symbolic_next) ? "*"
								: ((t.original_next != null) ? "node" + t.original_next.hashcode : "null")));
			} else
				return;
		}
	...
	static String printOriginalListCyclesString(Node head) {
		...
		for (Node t = head; t != null; t = t.original_next) {
			if (visited.add(t)) {
				result = result + "node" + t.hashcode + " elem = " + (Debug.getSymbolicIntegerValue(t.elem))
						+ " next = " + ((t._symbolic_next) ? "*"
								: ((t.original_next != null) ? "node" + t.original_next.hashcode : "null"))
						+ "\n";
			} else
	...
	static String printOriginalNode(Node t) {
		if (t == null)
			return "null";
		return "node" + t.hashcode + " elem = " + (Debug.getSymbolicIntegerValue(t.elem)) + " next = "
				+ ((t._symbolic_next) ? "*" : ((t.original_next != null) ? "node" + t.original_next.hashcode : "null"));
	}
