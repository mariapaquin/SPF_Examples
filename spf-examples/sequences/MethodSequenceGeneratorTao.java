...
public class MethodSequenceGeneratorTao {
	...
	public void invokeMethodSequenceDriver(String _className, int _numPublicMethods, int _sequenceLength, int _range) {
		...
		// OR
		//
		// if the user annotates the methods of interest, or explicitly specifies the
		// methods
		...
		// Reflection
		/*
		 * try { // from the class name get the actual class Class c =
		 * Class.forName(className); // get all public methods publicMethods =
		 * c.getDeclaredMethods(); for (int i = 0; i < publicMethods.length; i++)
		 * System.out.println(publicMethods[i].toString()); } catch (Throwable e) {
	...
	/**
	 * methodSequenceDriver is executed symbolically the arrays numSequencesDigits
	 * and inputDigits depend on the parameters numSequences and input
	 *
	 * let P = numPublicMethods, S = sequenceLength, and R = range then numSequences
	 * = P^S and input = R^P
	 *
	 * So numSequences is converted to base-P with S digits and input is converted
	 * to base-R with P digits
	 *
	 * We assume the input value is picked from the range specified in Preconditions
	 * annotation
	 *
	 */
